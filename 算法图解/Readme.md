...
层序遍历就是广度优先搜索，我们可以用队列来实现非递归的广度优先搜索。1. 将初始节点放入队列中 2. 从队列头中取出节点，查找该节点 3. 将该节点的直接子节点放入队列尾部 4. 重复2，直到找出满足要求的节点或队列为空。
若图中无循环，则不存在重复查找同一个子节点的情况，但若存在循环，必须要检查节点是否查找过，这可以简单地通过在节点上添加新的属性来完成，检查一个节点时，首先检查这个属性，如果这个属性为true, 就说明这个节点之前被检查过，不用再检查，也不用再将其直接子节点放入队列中，如果未检查过，就将这个属性设置为true, 然后进行检查。

第一层节点为一度关系，第二层为二度关系，以此类推。在广度优先搜索的执行过程中，搜索范围从起点开始逐渐向外延伸，即先检查一度关系，再检查二度关系，因此需要按添加顺序进行检查，否则找到的就不是最短路径（最近关系）。有一个可实现这种目的的数据结构，那就是队列（queue）。

对于检查过的节点，通过推入已检查队列、或者利用引用类型对象的特性更改节点标志的方式记录下来，否则可能导致统一节点被多次重复遍历，甚至是死循环。

### 图

散列表映射是图的一种实现。

散列表映射，用来描述相邻结点的关系。键值对代表关系，而值作为另一个键，再根据键值对查找散列表，这就构造了网络。散列函数将键变成下标，再在散列表中根据下表一步找到值，而这个值是一个键同样能被散列函数计算得到与它连接的（有关系的）结点，这是非常巧妙的结构。即，散列表的key是当前节点，值是指向的节点列表。


### 拓扑排序

如果任务A依赖于任务B，在列表中任务A就必须在任务B后面，并且A与B不一定紧邻。这被称为拓扑排序，使用它可根据图创建一个有序列表。

### Dijkstra算法

Dijkstra 只要求边的权重非负。我们假设所有边都是双向的，也就是说是环（更具体说，环一般用来表示无向图中两个节点的往返权重不一样，如果一样，用无向图表示即可），在边的权重非负时，每绕环一次（算完一个方向，再算反方向），总值肯定增加，该次计算会被舍弃，也就是说Dijkstra算法自带排除绕环特性，Dijkstra算法对有向无向图都适用。

但如果是有向无环图的话，单源最短路径可以拓扑排序一下之后线性求；如果是无向无环图的话，可以使用dfs算法；不一定非要使用Dijkstra。

没有经过堆优化的Dijkstra不能处理负权图。

#### 案例一

![负权图样例](https://img-blog.csdn.net/2018072615382315?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0twcm9ncmFt/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70)

对上图求从A到B的最短路，用Dijkstra算法，第一步就能得到所谓的最短路径长度`4`，不会去考虑边`(C,B)`，而实际上最短路径却需要加入边`(C, B)`，长度是`2`。

这是因为Dijkstra算法是贪心算法，大概前提是，从当前的所有可能中就能找到全局的最优解，而负权图不满足这个条件。

#### 案例二

还有一个案例用Dijkstra求解会出错，但目前并不清楚这种出错是不满足Dijkstra的哪个前提条件，案例如下图。

![案例](http://4picture.test.upcdn.net/superbed/2019/12/20/5dfc703176085c328917851e.jpg)

上述案例用Dijkstra求解的结果是`S>B>C>D>END`，总权重为`77`；但实际上最短路径为`S>A>D>END`，总权重仅为`39`，由于`A`节点在第一次选择最便宜的节点之后就被排除掉了，并且它只是起点的邻居，后续不会再涉及更新，因此就把实际上的最短路径排除掉了。

—— 读完第七章海报案例，更新如下 ——

上述案例可以通过Dijkstra来解，因为Dijkstra会在处理完一个节点后，找到最便宜的未处理节点。即，在处理完`B`节点后，会找到当前最便宜的未处理节点`A`，再来计算经过`A`节点到达其邻居的开销，也就能够找到正确的最短路径了。

#### 案例三

Dijkstra不适用于负权图，原因不在于上文说的不考虑剩余的边，而是因为在考虑的过程中如果遇到已处理过的节点，会跳过该节点，而对于已处理节点的舍弃有时意味着跳过了正确的最短路径。

看下面的例子：

![负权图](http://4picture.test.upcdn.net/superbed/2019/12/20/5dfc761376085c32891ba34d.jpg)

第一次处理，会找到海报为最便宜的未处理节点，因此更新经由海报到达其邻居架子鼓的开销，并更新海报为已处理，总权重为35。此时再找到未经处理的最便宜的节点，发现黑胶唱片，但黑胶唱片的邻居是海报，但海报实际上已经被处理过了。在Dijkstra中，节点一旦被处理，就意味着没有前往该节点的更便宜途径，但前往海报节点的更便宜途径实际上是存在的。

#### 总结

总得来说，狄克斯特拉算法这样假设：对于处理过的海报节点，没有前往该节点的更短路径。这种假设仅在没有负权边时才成立。在包含负权边的图中，要找出最短路径，可使用另一种算法——贝尔曼-福德算法（Bellman-Ford algorithm）。还有弗洛伊德算法也可以计算带负权图的最短路径，但是不支持带负权环的图。